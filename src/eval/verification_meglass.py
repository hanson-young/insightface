...
if __name__ == '__main__':
    ...
    parser.add_argument('--pairs-path', default='meglass_pairs.txt', help='meglass pairs path')
    ...
    for idx, item in enumerate(pairs_lists):
        ...
        if cos < 0.27 and int(item[2]) == 1:#best threshold is 0.27 in flw
            countt += 1
            # print(item, cos)

        if cos > 0.27 and int(item[2]) == 0:
            countf += 1
        # print(cos)
        # if(cos > 0.5):
        # print(item)
    print("tpr:%d,%f",countt,float(countt) / len(pairs_lists))
    print("fpr:%d,%f", countf,float(countf) / len(pairs_lists))
    ...
    print('[%s]Accuracy: %1.5f+-%1.5f' % ("meglass", acc, std))
